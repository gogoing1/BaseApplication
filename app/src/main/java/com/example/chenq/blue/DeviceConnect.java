		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BleService.ACTION_GATT_CONNECTED.equals(action)) {
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						Toast.makeText(DeviceConnect.this, "�豸���ӳɹ���",
								Toast.LENGTH_LONG).show();
					}
				});
			}
			if (BleService.ACTION_GATT_DISCONNECTED.equals(action)) {
				Toast.makeText(DeviceConnect.this, "�豸�Ͽ���", Toast.LENGTH_LONG)
						.show();
			...
			if (BleService.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
				...
				for (BluetoothGattService ser : gattServices) {
					...
					currentServiceData.put("Name", BlueUtils.attributes
							.containsKey(uuid) ? BlueUtils.attributes.get(uuid)
							: "Unknown Service");
