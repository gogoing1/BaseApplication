 ...
 * Desc: 高斯模糊工具类
 * 参考 https://www.jianshu.com/p/02da487a2f43
 */
public class BlurUtils {
    ...
    /**
     * 方案一
     * 使用 RenderScript 进行模糊计算
     * RenderScript是在Android上的高性能运行密集型运算的框架，RenderScript主要用于数据并行计算，尤其对图像处理、摄影分析和计算机视觉特别有用
     * RenderScript使用java代码调用 C++ 来处理高位图像运算，因此速度非常快
     * 缺点：底端机型存在适配问题，需要引入类库（但是它只能在API 17或者更高的版本使用）
     * Google 为了兼容低版本也可以用RenderScript，加了一个兼容包 android.support.v8.renderscript
     *
     * @param context
     * @param source
     * @param radius
    ...
    private static Bitmap rsBlur(Context context, Bitmap source, int radius, float scale) {
        // 优化策略，对原图进行缩小，使其丢失一部分像素点，然后在对它进行模糊处理，这样可以大幅度提升计算速度
